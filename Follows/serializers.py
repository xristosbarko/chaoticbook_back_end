...
class FollowingsSerializer(serializers.ModelSerializer):
    ...
    def get_follow(self, follow):
        ...
        check_follow = Follow.objects.filter(
            following=follow.following, follower=request.user
        )
    ...
    def to_representation(self, follow):
        representation = super(FollowingsSerializer, self).to_representation(
            follow
        )
        request = self.context.get("request")
...
class FollowersSerializer(serializers.ModelSerializer):
    ...
    def get_follow(self, follow):
        request = self.context.get("request")
        check_follow = Follow.objects.filter(
            following=follow.follower, follower=request.user
        )
        if check_follow.count():
            return {"sent": True, "pending": check_follow[0].pending}
        else:
    ...
    def to_representation(self, follow):
        representation = super(FollowersSerializer, self).to_representation(
            follow
        )
        request = self.context.get("request")
...
class FollowSerializer(serializers.ModelSerializer):
    user_id = serializers.PrimaryKeyRelatedField(
        source="following", queryset=User.objects.all()
    )
